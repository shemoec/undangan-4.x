...
export const guest = (() => {
    ...
    const showGuestName = () => {
        ...
        // Leer pases (si los necesitas luego)
        let passes = parseInt(params.get('passes') || '0', 10);

        // ----------- Mostrar saludo bonito ----------
        const guestName = document.getElementById('guest-name');
        if (guestName && name.trim() !== '') {
            guestName.innerHTML = `
                <div class="m-2 text-center">
                    <small class="mt-0 mb-1 mx-0 p-0">${guestName.getAttribute('data-message')}</small>
                    <p class="m-0 p-0 fw-semibold" style="font-size: 1.25rem">${util.escapeHtml(name)}</p>
                    ${
                        passes > 0
                            ? `<p class="m-0 small text-muted">(Total ${passes} pase${passes > 1 ? 's' : ''})</p>`
                            : ''
                    }
                </div>
            `;
        ...
        // ----------- Llenar solo el nombre limpio en el input ----------
